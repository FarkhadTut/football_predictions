...
- **Data-source reality (2026-06-14, REQ-006/007/012).** Two planned sources fell through and were replaced:
  ...
  - **Historical odds baseline**: Football-Data.co.uk was the planned source but carries **domestic leagues only** — no WC/Euro odds (probe claim falsified; see `reports/probes-phase0.md` correction). Replaced with an **OddsPortal browser-render + offline-cache loader** (`predictor.odds.oddsportal`). Got **1X2 for 4 tournaments** (WC 2018, Euro 2016/2020/2024). **WC 2014 gated** (OddsPortal hides >~12y odds behind login). **O/U 2.5 + BTTS deferred to Phase 1**: OddsPortal serves only 1X2 anonymously — the O/U/BTTS sub-market tabs redirect logged-out users back to 1X2, so those book odds need a logged-in session. The backtest uses its empirical-rate fallback for them meanwhile (verdict unaffected — model already loses on those markets).
  - **Backtest gate now runs with real numbers** (was `nan`) as an **expanding-window walk-forward** over the 4 odds-backed held-out tournaments (Euro 2016/2020/2024, WC 2018), each trained on all earlier tournaments. Pooled **n=123**. WC 2014 can't be a fold (nothing precedes it); WC 2022 has no data.
  - **Model tuned (2026-06-14)**: added neutral-venue mode (no home edge for international play) + **L2 ridge shrinkage** on team strengths + time-weighting (`model/dixon_coles.py`, tuned via `scripts/tune_model.py`). Ridge was the dominant lever — the unregularised MLE was badly overconfident on the ~3–7 matches/team per fold. This took the model from 34% worse than market to **near parity**: 1x2 1.34→**1.08**, ou_2_5 1.27→**1.02**, btts 1.17→**1.02**. Still **FAILs the gate** (0/3 markets ≤0.98). The baseline is the OddsPortal **consensus-average** closing line, which is efficient — a public-data goal model can match but not beat it on calibration.
  - **ROI / value backtest (2026-06-14)**: `scripts/roi_backtest.py` + `backtest/roi.py` simulate level-stakes 1X2 value betting (bet when `model_prob × odds > 1+edge`) at the raw OddsPortal odds over the 121 held-out matches with odds. Headline looks great (**+22–27% ROI**) — model *profitability* ≠ calibration. **But the breakdown shows it's a longshot artifact**: favourites (<2.5) **−1.6%** (no edge), mid (2.5–5) +10.7%, longshots (≥5.0) **+38.6%**. The "edge" is concentrated in high-variance longshots where the consensus *average* odds most overstate real payouts (real books cut longshot prices hardest) — and on a tiny 84-bet sample. So **not a robust money-making edge**: it reflects un-bettable average odds + variance, not a real exploitable signal. Consistent with the near-parity Brier result. 1xbet itself can't be compared: no historical odds + DNS-blocked here.
  - **Data bug fixed (2026-06-14)**: `teams` had 3-letter code duplicates (`ARG`/`BRA`/`ENG`/`FRA`) from ad-hoc WC 2026 demo fixtures (not the FBref loader — `fbref_id` was null). They only affected the 3 WC 2026 fixtures (not the backtest). Merged into the canonical full-name rows via `predictor.ingest.team_dedup` / `scripts/fix_team_code_dupes.py` (idempotent, collision-safe); WC 2026 fixtures now read Brazil/Argentina/France/England.
- **Phase 1 qualitative layer started (2026-06-15)**: the blending engine — the part that makes Claude notes actually affect predictions — is built. `model/qualitative.py` (`apply_note` / `apply_qualitative_deltas` + logit/sigmoid) folds a `ClaudeNote`'s per-market `log_odds_shift`s into `MarketMarginals` (1x2 symmetric home/away log-shift, ou/btts logit shift, corners skipped). `scripts/qualitative_demo.py` shows raw-vs-blended for any match. Phase 0 had only the notes *plumbing* (schema/ingest/SSE/UI) — notes were display-only. **Forward-only**: cannot be backtested (Claude knows historical results = hindsight leakage); value is assessed prospectively. Next: a real authored note + serving blended predictions via the API (the `/predict` endpoint is still enqueue-only).
