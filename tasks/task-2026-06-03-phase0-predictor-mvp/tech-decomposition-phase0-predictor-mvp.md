...
- **Data-source reality (2026-06-14, REQ-006/007/012).** Two planned sources fell through and were replaced:
  ...
  - **Backtest gate now runs with real numbers** (was `nan`) as an **expanding-window walk-forward** over the 4 odds-backed held-out tournaments (Euro 2016/2020/2024, WC 2018), each trained on all earlier tournaments. Pooled **n=123** across them. Model loses to the OddsPortal market baseline (1x2 1.34×, ou_2_5 1.27×, btts 1.17×) → **FAIL**, honestly (though it beats market on BTTS in Euro 2016 + WC 2018). WC 2014 can't be a fold (nothing precedes it); WC 2022 has no data. Beating the market is a Phase 1 modeling concern. Plan: `tasks/plans/steady-swinging-pine.md`.
