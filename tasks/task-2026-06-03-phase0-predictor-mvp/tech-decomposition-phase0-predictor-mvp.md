...
- [ ] Sub-step 2.1: [REQ-003] Models + initial migration
  - **Files / modules**: `apps/predictor/src/predictor/db/models.py`, `apps/predictor/migrations/`
  - **What changes**:
    - SQLModel models for `teams`, `players`, `wc_squads`, `matches`, `match_stats`, `odds_snapshots`, `predictions`, `score_distributions`, `model_runs`, `claude_notes` (metadata pointer only; payload is the file).
    - `model_runs(id PK, model_version, git_sha, training_cutoff_utc, fitter_config_json, created_at)`.
    - `predictions(match_id, market, model_run_id FK, probability, computed_at)` with composite uniqueness on `(match_id, market, model_run_id)`.
    - `score_distributions(match_id, model_run_id FK, matrix BLOB)` storing the 10x10 joint probability matrix as a numpy `.npy` byte buffer.
    - Indexes on `matches.kickoff_utc`, `odds_snapshots.match_id`, `predictions(match_id, model_run_id)`, `score_distributions(match_id, model_run_id)`.
    - Alembic config and first migration.
  - **Tests**: `tests/db/test_migrations.py` — migration up then down on a temp DB completes without errors; round-trip CRUD per entity.
  - **Depends on**: 1.1.
